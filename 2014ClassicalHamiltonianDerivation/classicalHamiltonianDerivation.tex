...
Fix a physical system for study. We define the set $\mathbbm{C}$ of all physically distinguishable configurations for that system. Each element $\mathbbm{c}$ we call \emph{configuration state}.
\end{defn}
...
Each classical configuration state $\mathbbm{c} \in \mathbbm{C}$ is a distribution over states: $\mathbbm{c}=\sum\limits_{\mathbbm{s} \in \mathbbm{S}} D(\mathbbm{s}) \mathbbm{s}$, where $D:I\rightarrow\mathbb{R}$ measures how much of the system can be found in each $\mathbbm{s}_i$. The distribution can be visualized as a histogram over the states in phase space.
\end{cor}

Under the classical assumption, we can then limit ourselves to the study of the infinitesimal elements, their states and their properties without loss of generality.

\begin{defn}\label{label}
To help identify states, we introduce the following: we call a \emph{label} a set of states $i\subset\mathbbm{S}$; a \emph{set of labels} a collection of disjoint labels $I | \forall i_1,i_2\in I, i_1\bigcap i_2 = \emptyset$; a \emph{state variable} a set of labels $X$ that covers all of phase space: $\bigcup\limits_{i \in X}i=\mathbbm{S}$. Therefore a state is in one and only one label of a state variable.
\end{defn}

\begin{defn}\label{labelsCombine}
Let $I_1$ and $I_2$ be two sets of labels. We can define the \emph{combined set}, $\langle I_1, I_2 \rangle$, whose labels consist of all the non-empty intersections of one label of $I_1$ and one of $I_2$. If all intersections are non-empty, $I_1$ and $I_2$ are said to be \emph{orthogonal}, and the total number of labels of $\langle I_1, I_2 \rangle$ is given by the product of the number of labels in $I_1$ and $I_2$.
\end{defn}
...
Let $S$ be the set of configuration states of a system that undergoes deterministic and reversible evolution. There exists a bijective map $f:S \leftrightarrow S$.
\end{prop}

\begin{cor}\label{detrevDist}
The evolution of a classical configuration state $\mathbbm{c}=\sum D(s) \mathbbm{s}$ under a bijective map is given by $\mathbbm{c'}=\sum D'(s) \mathbbm{s}=\sum D(f^{-1}(s)) \mathbbm{s}$.
\end{cor}
...
\begin{cor}\label{detrevDist}
Given a label $i$, the image $f(i)$ is also a label containing the same number of states. Given a set of labels $I$, the image $f(I)$ is also a set of labels containing the same number of labels. Given a state variable $X$, the image $f(X)$ is also a state variable.
\end{cor}

\begin{defn}\label{labelsIndep}
Let $I_1$ and $I_2$ be two orthogonal sets of labels. They are said to be \emph{independent} if and only if the image of the combined set $f(\langle I_1, I_2 \rangle)$ is equal to the combined set of the images $\langle f(I_1), f(I_2) \rangle$. It follows that $f(I_1)$ and $f(I_2)$ are orthogonal.
\end{defn}
