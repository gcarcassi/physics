...
\textbf{Discrete labels over $\mathbb{R}$}. Labels often will be numbers. For integers, there will be a one-to-one map between $z\in\mathbb{Z}$ and $i\in I$. Therefore we can use $I$ to both refer to the set of labels and the set of numbers they correspond to. For real numbers, each label will correspond to an interval.
...
With homogeneous labels no adjustment is needed, and the range can be used as a measure of the number of labels.\footnote{Note that homogeneous labels are not arbitrary: they depend on the map. Any non-linear transformation will transform homogeneous labels in non-homogeneous labels. Therefore either we require specific labels, and the map will tell us whether they are homogeneous, or require homogeneous labels, and the map will determine what they are, up to a linear transformation.}
...
A \emph{degree of freedom} is a label set $X$ given by a pair\footnote{Having exactly two variables for each d.o.f. is linked to the laws of physics being second order. It's justification will be part of a later work.} of homogeneous and orthogonal state variables $P$ and $Q$. We call these \emph{conjugate variables}.
\end{defn}
