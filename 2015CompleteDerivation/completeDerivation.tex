...
Moreover, the density itself has to be differentiable. The distribution $\rho_\mathcal{c}$ is a real valued function of the state. As such, at least locally, we can use it as a state variable $q^1=\rho_\mathcal{c}$. As a state variable it identifies the states by the density of the material associated with them. This is more physically meaningful as one may first suspect, since placing physical markers is a common way to define the references for a coordinate system, which in the end is placing material at particular states. In a coordinate system where $q^1=\rho_\mathcal{c}$, the function is clearly differential as $\partial_1 \rho_\mathcal{c} = 1$ and $\partial_i \rho_\mathcal{c} = 0$ for $i \neq 1$. As we change state variables, the transformation is differentiable, therefore all the derivatives $\partial_{i'} q^1 = \partial_{i'} \rho_\mathcal{c}$ exist. The density itself is differentiable.
...
\end{prop}
\begin{justification}
	We claim $\mathcal{S}$ is a differential manifold. Let $\rho_\mathcal{c} : \mathcal{S} \rightarrow \mathbbm{R}$ be the distribution associated with a composite state $\mathcal{c} \in \mathcal{C}$. Let $q^i$ and $\hat{q}^j$ two sets of coordinates. Let $\rho_\mathcal{c}(q^i)$ and $\rho_\mathcal{c}(\hat{q}^j)$ be the expression in local coordinates of the distribution.  $\rho_\mathcal{c}(\hat{q}^j)=\rho_\mathcal{c}(q^i) | \partial_j q^i |$ as it transforms as a density. $\rho_\mathcal{c}(\mathcal{s}) = \rho_\mathcal{c}(q^i(\mathcal{s})) = \rho_\mathcal{c}(\hat{q}^j(\mathcal{s}))$. The Jacobian $| d q' / d q |$ exists and is non-zero. The map between any two charts is differentiable. $\mathcal{S}$ is a differentiable manifold.
...
Note that neither of those expressions match the norm given by the vector product defined as $\langle f, g \rangle = \int_{\mathcal{S}} fg \, d\mu$. Moreover, since $\mathcal{C}$ excludes discontinuous functions, it can never be a complete normed space: it will not include the limit for all Cauchy sequences, it cannot be a Banach or a Hilbert space. Such constructions, though, are still useful. Consider the following expressions:
\begin{align*}
...
\rho_\mathcal{c}(q_0,p_0) &= \int_\mathcal{S} \delta_{q_0,p_0} \rho_{\mathcal{c}} d \mu = \langle \delta_{q_0,p_0} , \rho_{\mathcal{c}} \rangle \\
...
\right] 
\end{align*}
The Jacobian has to be unitary, therefore all elements $\partial_i \hat{k}^j$ must be well defined and $|\partial_i \hat{k}^j| = 1/d\hat{q}$. This only happens if $\partial_i \hat{k}^j$ is a $1\times1$ matrix: there is only one $k$. This gives $\partial_{k} \hat{k} = d_{\hat{q}} q$ and $\hat{k} = d_{\hat{q}} q \, k + f(q)$. As this is a change of units, we expect the zero value of $k$ to remain constant: $\hat{k}(q, 0) = 0 = f(q)$. Therefore $\hat{k} = d_{\hat{q}} q \, k$ transforms as the component of a covariant vector. $\mathcal{S}$ is isomorphic to $T^*\mathcal{Q}$ where $\mathcal{Q}$ is the manifold that defines our unit.
...
We can capture the above discussion by stating that the state space $\mathcal{S}$ is the symplectic manifold $(T^*\mathcal{Q}, \omega)$, where $\mathcal{Q}$ is the manifold that defines the unit system. This is the only manifold that allows us to define state-variable-invariant densities. The metric allows us to measure the cardinality of possibilities within and across independent d.o.f. 
...
\begin{justification}
	We claim the simplest state space $\mathcal{S}$ that allows state-variable-invariant densities is the cotangent bundle $T^*\mathcal{Q}$ of a single dimensional manifold $\mathcal{Q}$. Let $\mathcal{S}$ be the $n$ dimensional manifold for the particle states. $\mathcal{S}$ must be at least one dimensional to allow state-variable-invariant densities. Consider a set of state variables for $U \in \mathcal{S}$: the unit system for the simplest state space will be fully described by a single state variable. Let $q$ be the state variable that defines the unit system. Let $\mathcal{Q}$ be the manifold charted by $q$. $\mathcal{Q}$ is an embedded manifold of $\mathcal{S}$. $\mathcal{Q}$ is differentiable. A diffeomorphism $\hat{q}=\hat{q}(q)$ fully defines a unit transformation. Let $(q,k^i)$ with $i=1...n-1$ be a set of state variable. The Jacobian determinant of the transformation is $|J| = d_q\hat{q} |\partial_i\hat{k}^j|$. $|J|=1$ as densities must be invariant. $|\partial_i \hat{k}^j| = d_{\hat{q}}q$. $\partial_i \hat{k}^j$ is one dimensional as its components must be fully specified by the previous relationship. Let $k=k^1$. $\hat{k}(q,0) = 0$ as a change in units does not change the $0$ value. $\partial_k \hat{k} = d_{\hat{q}}q$. $\hat{k} = d_{\hat{q}}q k$. $k$ changes as the component of a co-vector. Each state is identified by a point and a co-vector in $\mathcal{Q}$. The simplest state space $\mathcal{S}$ is isomorphic to $T^*\mathcal{Q}$ where $Q$ is the one dimensional manifold that defines the units.
...
\begin{justification}
We claim the vector field $S \in T(T^*\mathcal{M})$ for the infinitesimal displacement $S^a = d\xi^a/ds$ admits a potential $\mathcal{H}$ such that $S^{a} \omega_{ab} = \partial_{b}\mathcal{H}$. The extended state space $T^*\mathcal{M}$ of a classical homogeneous material is a symplectic manifold by \ref{classical_extended_phase_space}. The map for infinitesimal evolution $\mathcal{T}_{ds}$ is an infinitesimal self-symplectomorphism by \ref{relativistic_canonical_transformation}. By \ref{symplectomorphism_generator} the infinitesimal displacement $S$ admits a potential $\mathcal{H}$ such that $S^{a} \omega_{ab} = \partial_{b}\mathcal{H}$
