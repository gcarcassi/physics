...
\begin{rationale}
	...
	The issues rising from a lack of perfect isolation are addressed. Each degree of freedom comes with its own unstated part (i.e. the motion of the fragments within that d.o.f.) therefore these is no pretense of perfect isolation. It is built into the description.
	...
	The inability to define time by following a deterministic and reversible problem is still present. The unstated part cannot give us a measurable quantity that changes in time as that would be a description of the unstated part.
	...
	In light of this, consider a muon decay: the initial state of the muon and the final states of the electron and neutrinos can certainly be well described by quantum particle states. We have no reason to expect, though, that the while muon is decaying into the resulting particles the state is always in equilibrium and therefore well described mathematically by a vector in a Hilbert space. After all, the state of a single spin one half particle cannot determine the future state of three spin one half particles.
	
	Using unitary evolution in those cases to make predictions, though, may still make physical sense. Quantities that are not affected by the particular evolution of the unstated part will have the same value under non-deterministic evolution and an equivalent quasi-static evolution. For example, the overall probability distribution resulting from the decay of a muon is something that cannot depend on the detail motion of the fragment (otherwise we would be able to characterize it). In these cases, the use of deterministic and reversible evolution as a ``stand-in" to calculate final statistical distributions is justified. But it should not be taken literally as a physical model of what actually happens. Because what happens is that, at a particular moment in time for causes the state cannot predict, the muon ceases to be a homogeneous material, and morphs into 3 inhomogeneous pieces, whose final state is not determined by the initial state. There is no justification for assuming this process to be quasi-static deterministic and reversible evolution of quantum particle states.
	
	In light of what we discussed, the irreducibility assumption is less flawed but it still cannot be fundamental, in the sense that we cannot take it to strictly apply to all systems. We should not expect to solve open problems, such as the arrow of time and the measurement problem, within the assumption.
\end{rationale}

\subsection{Quantum mechanics}

We are now ready to write the equation of motion for an irreducible material. Deterministic and reversible evolution for the whole distribution necessarily means quasi static evolution: there is a well defined state at each moment in time. Moreover the components must evolve independently as we cannot learn more about one component by observing its evolution in different combinations: the evolution of the composition is the composition of the evolution. In other words: the evolution must preserve the inner product.
