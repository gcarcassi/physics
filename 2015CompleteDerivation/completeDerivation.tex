...
While in principle, we could simply introduce assumption \ref{ass:irreducibility} and work our way to quantum mechanics, there is a non insignificant chance of being misconstrued. Different people come with different background, to the point that the same words trigger different intuitions and expectations. Therefore we will first ground in classical mechanics the intuition that will be needed later, making the step into quantum mechanics a gentler, and hopefully less confusing, one. It will also help us in showing that classical and quantum mechanics are not, after all, so different.

We will see that, as a first approximation, quantum particles are more keenly described by classical composite states (i.e. a distribution in phase space) as they are made of a finite amount of material. In fact, classical distributions already possess qualitative properties that are typically associated with quantum systems such as a classical analogue of the uncertainty principle.

\subsection{The case against point particles}

As we defined them in previous sections, classical particles should not be understood as point-like objects of finite mass. It was only by assuming they were infinitesimal parts of an invariant density distribution that gave us conjugate pairs of state variables. Hamiltonian mechanics would not have been justified otherwise.

Any finite amount of classical material is described by a distribution. Therefore, to a first approximation, an electron, which is a finite amount of charge/mass, must be described by a distribution as well. This may seem preposterous at first, but we contend that it is no more preposterous than to think it point-like, described by a probability distribution that self-interacts. In fact, the idea of an electron being a distribution of material is already conceptually more compatible with two issues.

The first one is that an electron does not seem to have a well defined position or momentum. If we misidentify the electron with a classical particle, this would indeed be worth of note and puzzlement. But if the electron is a classical distribution, there is not issue. The earth is both under Milano and New York. Your right index finger is not, hopefully, at the same position of your left eye. In fact, based on what we saw before, any object of finite mass is a distribution over a region, and discontinuous distributions are ruled out by \ref{continuous_map}. An electron is an object of finite mass: why should it be any different? In fact, if one claimed an electron to be point-like, he would be claiming an electron behaves in a different way than all other physical objects of finite mass (which are the only ones we experimentally work with). Shouldn't \emph{he} have the burden of proof?

The other issue is particle-wave duality. If the spread of the distribution is narrow enough for the problem at hand, the motion can be described by the position and center of mass, making it behave like a classical particle. If the spread is not negligible (e.g. it encounters obstacles of a smaller scale) then we have to use the full distribution. Therefore, even in this classical approximation, the electron would have this dual nature.
...
In fact, we can construct an infinitesimal object that is not elementary. Suppose we have a sphere with dots moving on its surface. All their dynamics can be expressed by their polar and azimuthal angles. If we scale the sphere, the dynamics stays the same. We can shrink the whole sphere to a point: it is now a point-like object that is not elementary, as there is motion on its surface detectable along different angle of incidence.
...
Suppose we have our classical electron described by a finite amount of classical material distributed over a single d.o.f. $\int \rho_\mathcal{c} dq \wedge dp = 1$ where $\rho_\mathcal{c}$ is the normalized distribution. In this context, we will call \emph{fragment} an infinitesimal part of our classical electron. The Shannon entropy associated with the distribution, which can be though as the amount of information required to identify a fragment given the distribution, is finite as well.\footnote{To be precise, Shannon's differential entropy is actually the number of bits needed relative to a uniform distribution of unitary range on all axis. The intuitive meaning, though, is the same.} $I(\rho_\mathcal{c}) = \int \rho_\mathcal{c} \log (\rho_\mathcal{c})  dq \wedge dp = I_0$. TODO: prove in appendix.

As the state evolves into its final state, the amount of material remains the same: $\int \rho_{\hat{\mathcal{c}}} dq \wedge dp = \int \rho_{\mathcal{c}} dq \wedge dp$. The final entropy will also remain the same: $I(\rho_{\hat{\mathcal{c}}}) = I_0$. Mathematically because $\rho_\mathcal{c} \log (\rho_\mathcal{c})$, like the distribution itself, is just another integrable function. Physically, because the amount of information required to identify a fragment given the distribution has to remain the same under deterministic and reversible evolution. Therefore, in the same way that each fragment is constrained to move along states at equal energy, the composite state is constrained to move along distributions at equal entropy. 
...
\rho = &\frac{1}{ \sqrt{4 \pi ^2\sigma_q^2\sigma_p^2}} e^{-\frac{(q-\mu_q)^2(p-\mu_p)^2}{4\sigma_q^2\sigma_p^2}} \\
= &\frac{1}{e^{I_0 - 1}} e^{-\frac{\pi ^2(q-\mu_q)^2(p-\mu_p)^2}{\exp (2(I_0 - 1))}}
\end{align*}
