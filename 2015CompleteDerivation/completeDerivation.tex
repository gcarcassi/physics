...
Note that the isomorphism is to a subspace of continuous functions as we may have to exclude, for example, those that are not bounded or those that do not vanish at infinity. These may, in some cases, not represent physically meaningful system.

\subsection{Coordinate invariant densities}

Math: need linear functional from $\rho$ to total amount of material. For Riesz representation theorem, there can be only one. Density must be invariant under coordinate transformation. Density live on $T^*\mathbbm{Q}$. Must be able to define marginal distributions on submanifold. Metric is two form. Metric must be form invariant under coord transformation. Metric is canonical two form.

Physics, 1D, show that densities do not work on Q. Show they work on T*Q. Define degree of freedom. Independent degree of freedom are orthogonal. Area in each d.o.f. is number of possibilities. Total number of states is product of number of possibilities for independent d.o.f. Independent d.o.f. are orthogonal. Leads to omega metric. Density allows to count particle states. Example: two uniform distribution for the same amount of particles. 
...
The state space $\mathcal{S}$ for the particles of a homogeneous classical  system is a cotangent bundle $T^*\mathbbm{Q}$ with symplectic metric $\omega$.
\end{defn}
