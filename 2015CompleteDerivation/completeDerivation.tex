...
\begin{rationale}
	The idea is that time evolution specifies a map for the state space $\mathcal{S}$ of the infinitesimal parts. And knowing how the parts evolve, we also know how to evolve the whole, that elements in $\mathcal{C}$.
	
	Consistently with what we said in Assumption \ref{detrevass}, if we defined a state for the infinitesimal parts, then one such deterministic and reversible evolution should exist. Yet, the idea that we can assign states to infinitesimal parts should be considered only a \emph{simplifying} assumption, and it is instructive to understand when it breaks down.
