...
The problem goes along these line. One assumes, implicitly or explicitly, that quantum particles are point-like. But they don't behave like one, they are described by a distribution. In some cases, that distribution experimentally becomes a probability distribution. So the state of a quantum particles is a probability distribution. Except that it self-interacts. So its a weird quantum probability distribution. The quantum particle behaves like its in more than one place at once. We learn this, we get used to it, we can get our calculations done... but it's still preposterous.

The first thing to note here is that most objects behave like they are in more than one place at once. The earth is both under Milano and Ann Arbor. The readers right index finger is probably at a different place from his left eye. In fact, based on what we saw before, any object of finite mass is a distribution over a region, as discontinuous distributions are ruled out. An electron is an object of finite mass. Why should it be any different? In fact, this we are claiming that is different from any other object, shouldn't there be a high burden of proof?

Part of the confusion seem to be the idea is that the electron is an elementary particle, and therefore should have no spatial extent. But that does not follow. First of all, because one can construct a counter example. Suppose you have objects constrained on the surface of a sphere. All their dynamics can be expressed by their polar and azimuthal angles. If we scale the sphere, the dynamics stays the same. We can shrink the whole sphere to a point: it is now a point-like object that is not elementary. Second of all, if elementary objects are point-like, they cannot know how many spatial dimensions are there. Now can the whole universe be reduced to elementary elements that have no idea how many spatial dimensions are there?

Entertaining the thought that an electron is not a point-like object described by weird quantum probability, but an amount of material spread in space like any other, may seem, at this point, weird. Mainly because its not how we are taught to think about it, and trying out a new thought is hard work. But hopefully it should be evident that this new thought is not at all more preposterous than the old one.

In fact, if we accept it, we start to see that another critical feature usually associated with quantum systems is already there for classical distributions.
...
As the state evolves into its final state, the amount of material remains the same: $a_m = \int \rho_{\hat{\mathcal{c}}} dq^n \wedge dp_n$. The final entropy will also remain the same since, like the distribution itself, is just another integrable function: $I(\rho_{\hat{\mathcal{c}}}) = I_0$.
...
We now ask the following question: once we fix the entropy, what is the relationship between the spread of the marginal distributions along $q$ and $p$? What is the distribution $\rho$ that minimizes the product of the variance $\sigma_q^2 \sigma_p^2 = \int (q-\mu_q)^2 \rho \, dqdp \int (p-\mu_p)^2 \rho \, dqdp$?

Suppose that we let interact it with an entropy source for enough time. The marginal entropy
