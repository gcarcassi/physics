...
\begin{prop}\label{prop:wavefuntion}
	The state space $\Psi$ for an irreducible material is isomorphic to the space of Lebesgue square integrable differentiable functions over $\mathcal{Q}$. That is $\Psi \cong C^1(\mathcal{Q}) \cap L^1(\mathcal{Q}, \mu)$. The state space for a quantum particle is the projective space $\mathsf{P}(\Psi)$.
\end{prop}
\begin{justification}
	TODO
\end{justification}

The observation made after \ref{prop:differentiable_manifold} in classical case are still valid: $\Psi$ is not a complete metric space. 
