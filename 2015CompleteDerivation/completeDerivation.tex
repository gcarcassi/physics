...
Note that neither of those expressions match the norm given by the vector product defined as $\langle f, g \rangle = \int_{\mathcal{S}} fg \, d\mu$. Moreover, since $\mathcal{C}$ excludes discontinuous functions, it can never be a complete normed space: it will not include the limit for all Cauchy sequences. Such constructions, though, are still useful. Consider the following expressions:
\begin{align*}
\Lambda_{\mathcal{S}} (\mathcal{c}) &= \int_\mathcal{S} \rho_{\mathcal{c}} d \mu = \int_\mathcal{S} 1\cdot  \rho_{\mathcal{c}} d \mu \\
\Lambda_U (\mathcal{c}) &= \int_U \rho_{\mathcal{c}} d \mu = \int_\mathcal{S} 1_U \cdot \rho_{\mathcal{c}} d \mu \\
p_{total}(\mathcal{c}) &= \int_\mathcal{S} p \cdot \rho_{\mathcal{c}} d \mu \\
q_{avg}(\mathcal{c}) &= 1/|\rho_{\mathcal{c}}| \int_\mathcal{S} q \cdot \rho_{\mathcal{c}} d \mu
\end{align*}
For each state, they represent respectively the total amount of material, the material within $U$, the total momentum and the average of a state variable. As they are linear functionals of $\mathcal{C}$,  they can be expressed using their dual vector over $\mathcal{S}$. But these vectors ($1$, $1_U$, $p$, $q/|\rho_{\mathcal{c}}|$) are \emph{not} elements of $\mathcal{C}$. They do not represent states, they are not continuous and integrable, and there is no single general physical meaning for them. Therefore, as we mentioned before, while we can extend the function space for convenience, computation purposes or to study a limit case, we need to be mindful of the extension and carefully check that the mathematical results are physically meaningful case by case.
...
\right] 
\end{align*}
The Jacobian has to be unitary, therefore all elements $\partial_i \hat{k}^j$ must be well defined and $|\partial_i \hat{k}^j| = 1/d\hat{q}$. This only happens if $\partial_i \hat{k}^j$ is a $1\times1$ matrix: there is only one $k$. This gives means $\partial_{k} \hat{k} = d_{\hat{q}} q$ and $\hat{k} = d_{\hat{q}} q \, k + f(q)$. As this is a change of units, we expect the zero value of $k$ to remain constant: $\hat{k}(q, 0) = 0 = f(q)$. Therefore $\hat{k} = d_{\hat{q}} q \, k$ transforms as the component of a covariant vector. $\mathcal{S}$ is isomorphic to $T^*\mathcal{Q}$ where $\mathcal{Q}$ is the manifold that defines our unit.
