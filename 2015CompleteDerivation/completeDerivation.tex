...
\begin{prop}\label{discrete_state_metric}
	Let $U \subseteq \mathcal{S}$ a set of states fully identified by a set of $n$ independent discrete state variables $q^i$. Let $\Delta q^i = q^i(U)$ the range of possibilities of each variable. Then $\#(\Delta q^i)=\#(\Delta \hat{q}^i) \; \forall i$ and $\#(U)=\prod\limits_{i=1}^{n}\#(\Delta q^i)=\prod\limits_{i=1}^{n}\#(\Delta \hat{q}^i)=\#(\hat{U})$ where $\hat{U}=\mathcal{T}_{\Delta t}(U)$ and $\#$ denotes the cardinality of the given set.
\end{prop}
...
We claim $\mathcal{C}$ is an additive group. As we want to describe changes during the evolution (i.e. $\hat{\mathcal{c}} = \mathcal{c} +\delta \mathcal{c}$) we introduce an inverse $- : \mathcal{C} \rightarrow \mathcal{C}$ such that $\mathcal{c} + ( - \mathcal{c}) = 0 \forall \mathcal{c} \in \mathcal{C}$. Such inverse may introduce objects that do not properly represent a physical state, but a state change. A change of a physically distinguishable object is itself physically distinguishable therefore $\mathcal{C}$ is still a Hausdorff and second countable topological manifold as per \ref{topologically_distinguishable}. We'll still call $\mathcal{C}$ state space committing an abuse of terminology.
\end{justification}
...
As we combine parts, the total amount of material will be the sum of the material of the parts. As we increase or decrease by a constant factor the total amount of material in a state, we will find the total amount of material of that state increased or decreased by that factor. That is: $\Lambda_U(a_1 \mathcal{c}_1 + a_2 \mathcal{c}_2) = a_1 \Lambda_U(\mathcal{c}_1) + a_2 \Lambda_U(\mathcal{c}_2)$. For a proper state, one that is not the difference of two states, we will expect a positive amount of material. So, if a density $\rho_\mathcal{c}$ is positive everywhere, then the total amount is also positive. This makes $\Lambda_U$ a positive linear functional. As the amount of material we will measure is always finite, the functional applied to any composite state $\mathcal{c}$ over any set $U$ will be finite.
...
On a state space with discrete topology and a single state variable we simply have $\Lambda_U (\mathcal{c}) = \sum \limits_{q=a}^b \rho(q)$. For states identified by a single continuous state variable we would expect $\Lambda_U (\mathcal{c}) = \int_a^b \rho (q) dq$. By a simple change of state variable, we see that this does not work in general: $\rho(q')= \rho(q) dq/dq'$. This would make $\rho$ a function not just of the state $\mathcal{s}$, but also of the state variables we are using. This is inconsistent with our previous definition. Moreover if the transformation is not differential, the density is not even well-defined. This tells us that $\mathcal{S}$ cannot be any manifold: it has to be one that allows state-variable-independent densities. Under arbitrary change of state variables, the density has to remain the same, which means that the Jacobian of the transformation must exist and must be unitary.

For the Jacobian to exist $\mathcal{S}$ has to be a differentiable manifold; that is the transformation between state variables is always differentiable. This way, at least, the density is well defined for any choice of state variables.

If $\rho$ is to remain unchanged and the Jacobian to be unitary we cannot simply change one state variable as we please. If we change one at least another has to change in some coordinated way such that the Jacobian of the total transformation is unitary. This means that we cannot change physical units of all the variables as we please: they are part of a unit system. For example, changing units of position will also change units of momentum. Let $\mathcal{Q}$ be the manifold charted by the minimum set of state variables that define such system of units. An element of $\mathcal{S}$ will be a geometrical object in such space. As these are states for infinitesimal parts, it will be a local object in $\mathcal{Q}$. As we saw before, it cannot be a point as the density $\rho(q')= \rho(q) dq/dq'$ is not invariant. It cannot be a vector $v(q) e_q$, as the density $\rho(q', v')= \rho(q, v) (dq/dq')^2$ is not invariant. A covector $k(q) dq$ is instead fine as $\rho(q', k')= \rho(q, k) (dq/dq') (dq'/dq) = \rho(q, k)$. In other words: a covector is the simplest local geometrical object that allows coordinate independent densities. We can identify $\mathcal{S}$ as the cotangent bundle $T^*\mathcal{Q}$, the space of all possible covectors.
...
This gives us insight on the physical meaning of the geometrical structure $T^*\mathcal{Q}$. The canonical one-form $\theta=k(q) dq$ represents the geometrical object we associate with each particle state, which is not a point in $\mathcal{Q}$. The canonical two-form $\omega=\Sigma \, dq^i \wedge dk_i$ is the metric that quantifies the possibilities described by two given state variables. Note, instead, that the relationship $\omega = - \hbar d\theta$, while mathematically true, has no clear physical meaning as we have not defined what the exterior derivative on a state actually represents.
...
The state space $\mathcal{S}$ for the particles of a homogeneous classical  system is a symplectic manifold formed by a cotangent bundle $T^*\mathcal{Q}$ equipped with the canonical two-form $\omega$.
\end{defn}
...
Now that we have fully characterized what we mean by a classical system, we can now stipulate the following:
...
\begin{rationale}
	...
	The first problem is methodological. To be able to talk about the states of a part we need a physical process that is able to distinguish between them. For a pool ball we can imagine marking one spot with a different color. This allows us to track it as the ball moves or collides with other balls. For an electron, instead, we do not have a way to mark a piece. In fact when two electrons scatter we can't even tell which is which, let alone what part went where. The classical assumption may not hold because we do not have suitable physical processes at our disposal.
	
	The second, more conceptual, problem is that the idealization that allow us to hold Assumption \ref{detrevass} break down as the parts get smaller. One case where a system is sufficiently isolated to define a state is when it is "big enough". Recall the cannonball whose motion is sufficiently unaffected by the photons that scatter off its surface. Clearly this breaks down for infinitesimal parts: as we keep dividing the material, we will find an amount that will be affected by the interaction with the environment, and therefore not sufficiently isolated. The other case where Assumption \ref{detrevass} holds is where the interaction is not symmetrical. For example, suppose the electric charge of the system under study is much larger than the charges in the environment. The electromagnetic field generated can be considered determined only by our system with good approximation. But this also breaks down: if part of the environment is made of the same homogeneous material the charge density will be the same and the interaction between particles will be symmetrical.
	
	In other words: the definition of a state requires some kind of asymmetry between system under study and environment. If we try to assume that the whole universe is really made of infinitesimal homogeneous parts, then the claim to that asymmetry is lost. Therefore our justification for holding the assumption has to be that the pieces are small compared to some scale but large compared to another. For example, water is a homogeneous material if we consider it made up of parts sufficiently small for the scale of the problem at hand but big enough to contain a large number of molecules.

	Other conceptual problems stem from the following two competing requirements: on one side we want the system under study to be isolated so that we can define independent states; on the other side we do not want the system isolated so that we can define physical process on it. For example, if we were to take the state of an infinitesimal part to really be its full description, with no unstated part, then each piece would evolve independently, oblivious to the other parts. Each particle would essentially live in its own separate universe.
	
	If we assume the whole system is deterministic and reversible but each part is not, then their evolution can depend on the states of the other parts. But this does not solve the problem entirely. Let's ignore the question of how we were able to define the states of the parts. Let's also ignore that the type of forces one would typically use (i.e. ones that conserve energy or ones with scalar and vector potentials) are precisely the ones we derive by assuming the pieces are not interacting with each other. The problem is that we can always locally separate the evolution into independent degrees of freedom. For example, the position and momentum of two particles may effect each other during the evolution, but the average and difference in position and momentum may evolve independently.\footnote{Also note that any Hamiltonian is locally isomorphic to a free particle (citation needed)} We can always find such local decomposition, and the easiest way too see that is in terms of the transported variables: they retain the original value, they clearly evolve independently. And since the pieces are infinitesimal, a local decomposition is all that is needed. While such description may be cumbersome to achieve in practice, conceptually is still possible. So, even if the particles are in principle interacting, the system is described by degrees of freedom that evolve independently, oblivious to each other.
	
	The same type of problem arises on the whole system when we consider its interaction to our probe or other parts of the environment. Say $a \in A$ is the state of our system and $b \in B$ the state of our probe, the future state $\hat{a}=f(a)$ because the system is deterministic, but $\hat{b}=g(a,b)$ because the probe needs to be affected by the system. This means $(a, b) \mapsto (\hat{a}, \hat{b})$ is not a reversible map. In other words: we can't expect to have deterministic and reversible evolution at all levels of decomposable systems where the parts interact in any physically meaningful way.
	...
	In the light of what we discussed, we cannot take the classical assumption to be fundamental, in the sense that we cannot take it to apply to all of physics. While ultimately flawed, the  assumption can be considered valid for a great number of macroscopic system, and that is why is very useful. Understanding its shortcoming will help us later to see how the quantum assumption solves, at least partially, some of these issues.
	
	As a final note: we caution in automatically thinking that the classical assumption is valid for all macroscopic systems. It is conceptually possible to have a macroscopic system where a clear independent state cannot be assigned to each part, in which case the assumption would not hold.
\end{rationale}


\subsection{Hamiltonian mechanics}

We are finally ready to write the equations of motions. As per \ref{detrevmap} our evolution is at least a self-homeomorphism $f:\mathcal{S} \leftrightarrow \mathcal{S}$. But this is not enough.

The evolution must map the distribution point-wise so that the value associated at the initial state is the same as the value. All of the system that starts in $\mathcal{s}$ has to end up in $\hat{\mathcal{s}}$. In math terms $\rho_{\hat{\mathcal{c}}} (\hat{\mathcal{s}}) = \rho_\mathcal{c}(\mathcal{s})$.

In the same way, if $U \in \mathcal{S}$ is a set of initial particle state and $\Lambda_U(\mathcal{c})$ is the amount of material associated to that set, then we expect it to be equal to the amount of material $\Lambda_{\hat{U}}(\hat{\mathcal{c}})$ associated with the set of final states $\hat{U} \in \mathcal{S}$.

But probably the best way to look at it is that initial and final set of states have to contain the same cardinality of states and possibilities, which where defined by our metric $\omega$. Therefore the area in each degree of freedom, which represents the number of possibilities in said d.o.f., needs to be mapped to an equal area in the transported d.o.f. (i.e. the d.o.f. defined by the transported state variables). Independent d.o.f. must remain independent, and therefore transported orthogonal d.o.f. remain orthogonal. This means that the product of possibilities of independent d.o.f. is also conserved. These statements are the physical justification of the Gromov's non-squeezing theorem and Liouville's theorem. And they give intuitive insight on the geometry of Hamiltonian systems.

Mathematically, under the classical Hamilotnian assumption,  deterministic and reversible evolution is an self-isometry. That is, it does not just preserve the topology but also the metric.

\begin{defn}\label{canonical_transformation}
	A deterministic and reversible evolution map for a homogeneous classical material is a canonical transformation and self-isometry, $\mathcal{T}_{\Delta t}:(T^*\mathcal{Q}, \omega) \rightarrow (T^*\mathcal{Q}, \omega)$.
\end{defn}

\begin{justification}
	We claim $\mathcal{T}_{\Delta t}$ is a self-homeomorphism. $\mathcal{T}_{\Delta t}$ is a deterministic and reversible evolution map and \ref{detrevmap}.
	We claim $\mathcal{T}_{\Delta t}$ is an self-isometry on $(T^*\mathcal{Q}, \omega)$. The Jacobian for $\mathcal{T}_{\Delta t}$ has to exist and be different from zero in order for the distribution on final states to be defined. $\mathcal{T}_{\Delta t}$ is a diffeomorphism.  $\omega$ is the metric for the cardinality of possibilities. A deterministic and reversible process conserves the number of states and possibilities. $\omega$ is invariant under a deterministic and reversible evolution map.
\end{justification}
