...
\item Classical Hamiltonian mechanics describes the evolution of a homogeneous material for which the state is equivalent to the states of all its infinitesimal parts, the classical particles. The state space for classical particles is the symplectic manifold $(T^*\mathcal{Q}, \omega)$; deterministic and reversible evolution is a symplectomorphism (i.e. a canonical transformation).

\item If the classical particle states are fully characterized by their motion in space, then the states also evolve according to Lagrangian mechanics and the motion is that of a massive particle under scalar/vector potential forces.
...
\item Quantum (Hamiltonian) mechanics describes the evolution of a homogeneous material for which the state can only be reduced up to finite parts, the quantum particles, whose internal motion is undetermined. The state space for quantum particles is an inner vector product space $(\Psi, \langle \cdot , \cdot \rangle)$; deterministic and reversible evolution is a unitary transformation.
\end{itemize}

While we are sure that some mathematical details can be improved, the feel that the overall ``conceptual castle" is solid. The starting concepts are simple, the conceptual mapping of those concepts into math is often straight forward and it often gave intuitive ideas that connects to different branches of math and physics: it cannot be a simple coincidence.

While the scope of this work is limited to re-founding and re-understanding the already known physical theories, it is our hope that this insight may also lead to new ideas to address some of the open problems in the foundations of physics.
