...
While some branches of physics (e.g. Newtonian mechanics, thermodynamics, special relativity) are derived from physical principles (e.g. Newton's laws, laws of thermodynamics, invariance of the speed of light), others start by simply setting a mathematical framework. It is the case for classical Lagrangian mechanics, where we start by assuming the existence of a Lagrangian function whose path integral is minimized along the motion. It is the case for classical Hamiltonian mechanics, where we start by assuming the state is described by conjugate pairs $(q^i, p_i)$ and evolves according to Hamilton's equations. It is the case for quantum mechanics, where the state space is a complex inner product vector space and the state evolves according to Schroedinger's equations. That is, at the very core of some of our physical theories we have mathematical definitions instead of physical ideas.

If we ask ``when is a particular physical system suitably described by Hamiltonian (or by classical) mechanics?" the only answer we have is ``when it fits the mathematical framework". While many seem to be content with this answer (e.g. ``you have to start from somewhere, it may as well be the math which at least is precise"), it begs the question: it does not tell us how and what physical properties of the system under study are being captured.

The fundamental problem is that mathematical structures are not enough to characterize the physical system being described. For example, the same math of linear circuits can be used to describe electric, hydraulic, thermal or mechanical systems. The mathematical treatment is the same because it captures aspects of the description that are common to all cases. But for the same very reason, we cannot infer what system is being described if all we are given is the mathematical model. In the jargon of linguistic/computer science/phylosophy: math captures the syntax (the relationships between the objects) but does not capture the semantic (the meaning of each object).

We started this work with the unpretentious goal of exploring simple questions such as the following. What do these mathematical symbols used in classical/quantum Hamiltonian represent? What is the physical meaning attached to it? Why is the state of a quantum system described by a wave function (i.e. a vector in a complex inner product space), while the state of a classical hamiltonian system is described by the three pairs $(q^i, p_i)$ (i.e. a point in a cotangent bundle)? The idea was to create a math-to-physics dictionary so that each mathematical concept was clearly mapped to a crisp physical concept. But to make sure that such dictionary was complete, we realized we had to make sure that the whole relevant mathematical framework could be recovered.

This redefined our mission to re-deriving as much as possible starting from physical assumptions. It also forced us to push further and further back toward the basic definitions. For example, if we truly wanted to justify why the state space of Hamiltonian mechanics is charted by conjugate variables $q^i$ and $p_i$, we had to justify why it was a manifold as well, which in turn meant we needed to justify why it was a topological space. But when we did find physical concepts that mapped to these more basic mathematical definitions, we realized that this link was valid for a far wider range of cases: the concepts were more general than we anticipated. For example, we realized that the mathematical notion of a topology maps incredibly well to the idea of physical distinguishability, but states of Hamiltonian system are not the only thing that are physically distinguishable.

To our surprise, this naturally expanded the boundary of the work. The same link that allowed to justify Hamiltonian dynamics also gave us relativistic motion on the extended phase space. The same link that allowed to justify Lagrangian mechanics also onstrained the motion to that of free particles under scalar/vector potential forces. We also realized that sometimes different mathematical concepts could be justified from the same physical starting point. That is: by putting physics first we could achieve a sort of unification of ideas that was not practicable by math alone.

The success of the approach lead us to think that, maybe, the picture we were developing was not simply ``a nicer way to think about things", but instead a better framework to understand and link different parts of basic science and mathematics. After many years of work and different incremental iterations we think it is time to checkpoint and report the progress.

The goal of this paper is to show that it is possible to derive classical/quantum Hamiltonian/Lagrangian mechanics starting from a few physical principle. We do \emph{not} claim that we have perfectly achieved this result, with no room from improvement. We do claim that the whole architectural framework is solid enough to be convinced that this is indeed possible, and that it is a worthwhile endeavor as it allows to make intuitive connections between different areas of physics and mathematics. That is: it \emph{can} and \emph{should} be done.

While trying to re-derive classical, quantum and (to some extent) relativistic mechanics may seem too broad a scope, it is necessary to ensure that the concepts we develop are truly general, that they can be applied to many different cases. If we could derive classical Hamiltonian mechanics on assumptions that are inapplicable to the quantum case, what good would it do? Quantum system are Hamiltonian systems, so clearly we haven't understood what makes a system Hamiltonian. The ability to apply the same physical ideas far and wide is what gives us confidence that these are truly general concept that capture the physical aspects we are describing. The breath itself is what makes the work interesting and more than a curiosity.
...
In section \ref{sec:dynamical_systems} we will show how the physical concept of physical distinguishability maps to the mathematical concept of topological spaces. Deterministic and reversible evolution is an invertible continuous map (i.e. a homeomorphism) as it needs to preserve what is physically distinguishable.
...
In section \ref{sec:Lagrangian} we will introduce the assumption of kinematic equivalence, studying the motion in space is equivalent to studying the evolution of the state. Under that assumption there is a link between the state variables $(q^i, p_i)$ and the kinematic variable $(x^i, u^i)$ (i.e. position and velocity). This link will allow us to express the number of possible initial conditions for d.o.f. in terms of $dx^i$ and $du^i$ (leading to the metric tensor $g$). Such expression also allows to constrain the Hamiltonian to the one for massive particles under scalar potential forces and an equivalent Lagrangian formulation.
...
The overall layout of the work is a balance between the need to show that the derivation follows (which requires starting from basic principles and reaching conclusion using formal arguments) and the need to give some intuitive understanding (which often requires starting from specific cases and then generalizing).
...
As these sections are clearly demarcated, this allows us to make sure that each justification only references past propositions. It also makes it easier to read the work at the desired level of detail.
...
The work uses ideas and tools from a broad range of mathematical and scientific subjects: topology, measure theory, differential geometry (symplectic and Rimannian), group theory and vector spaces, statistics, information theory, Hamiltonian mechanics, Lagrangian mechanics, quantum mechanics, thermodynamics, special relativity and so on. Connecting these areas, in fact, is part of the insight given by this work. While this is a necessity, it does create a number of problems.

The first one is terminology and prerequisite knowledge. Since we touch a good number of areas, we cannot expect all readers to be equally comfortable with all the technical terms. Yet we cannot introduce all of them either. The compromise we adopted is to keep the informal discussion more accessible while keeping the formal derivation more precise. Even in the formal derivation, though, we will seldom need more than the basic definitions within a specific area and few major results.

The second problem is notation. As each field has developed its own set of conventions, the common usage is now an empty set. We ended up with a compromise that we believe strikes a good balance. Especially in the informal discussion, we crafted something that in general feels familiar to a physicist. Within formal parts, though, we may use notation and terminology more specific to the area of math being used. But then again: one would need to be familiar to that area anyway to be able to follow the details.

We have tested the prose on a number of students and colleagues to be confirm its accessibility at different levels. We hope the reader enjoys this journey and comes out with a few new insights and ideas.
