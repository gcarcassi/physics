...
We claim $\mathcal{C}$ is an additive group. As we want to describe changes during the evolution (i.e. $\hat{\mathcal{c}} = \mathcal{c} +\Delta \mathcal{c}$) we introduce an inverse $\mathcal{c}_2$ for each element $\mathcal{c}_1$ such that $\mathcal{c}_1 + \mathcal{c}_2 = 0$. The inverse may not represent a proper physical state, yet we'll still call $\mathcal{C}$ the state space committing an abuse of terminology. The inverse $- : \mathcal{C} \rightarrow \mathcal{C}$ is physically unique and well defined.
\end{justification}
