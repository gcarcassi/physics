...
\begin{prop}\label{prop:inner_product}
	The state space $\Psi$ for an irreducible material is an inner product $\langle \cdot |\cdot \rangle : \Psi \times \Psi \rightarrow \mathbbm{C}$.
\end{prop}
\begin{justification}
	TODO
\end{justification}
...
\begin{proof}
	We claim $\langle x , y \rangle = \langle x, \mathsf{P}_x(y)\rangle$. Let $x, y \in X$. $\langle x , y \rangle x = |x|^2 \mathsf{P}_x(y) = |x|^2 \mathsf{P}_x(\mathsf{P}_x(y)) = \langle x, \mathsf{P}_x(y)\rangle$.
	
	We claim $\mathsf{P}_x(y) = a x$ for some $a \in \mathbbm{C}$. Let $x, y \in X$. $\mathsf{P}_x(\mathsf{P}_x(y)) = \mathsf{P}_x(y)$. $\mathsf{P}_x(y) = a x$ for some $a \in \mathbbm{C}$
	
	We claim $\langle a x , y \rangle = a^\dagger \langle x , y \rangle$. Let $x, y \in X$ and $a \in \mathbbm{C}$. $\langle a x , y \rangle = \langle a x, \mathsf{P}_x(y)\rangle = \langle a x, b x\rangle$ for some $b \in \mathbbm{C}$. $\langle a x, b x\rangle = \frac{b}{a} \langle a x, a x\rangle = \frac{b}{a} |a x|^2 = \frac{b}{a} |a|^2 |x|^2 = \frac{a^\dagger ab}{a} \langle x, x\rangle = a^\dagger \langle x, b x\rangle = a^\dagger \langle x, \mathsf{P}_x(y)\rangle = a^\dagger \langle x , y \rangle$
\end{proof}

\begin{thrm}
	Let $C^1(\mathcal{Q}, \mathbbm{C})$ the space of differentiable complex functions defined over $\mathcal{Q}$. Let $q : \mathcal{Q} \rightarrow \mathbbm{R}$ a differentiable function. Let $K_q$ a covariant linear operator: given an invertible differentiable function $\hat{q} : \mathbbm{R} \rightarrow \mathbbm{R}$ $K_{\hat{q}} = \partial_{\hat{q}} q K_{q}$. Then $K_q = a \partial_q$ for some $a \in \mathbbm{C}$.
\end{thrm}
