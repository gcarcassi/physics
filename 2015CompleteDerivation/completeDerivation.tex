...
\begin{rationale}
	...
	To be clear: we are not saying that kinematic equivalence is fundamental because everything must reduce to a material with no internal dynamics. That is: we are not saying elementary materials are minimal. While this statement seems reasonable and may fit our physical intuition, we have no a priori argument for it. We can imagine, in fact, that as we reduce the behavior of one material to its constituents we find the parts to possess richer and richer states. In a similar way that the elementary particles have electromagnetic and color charges, while their aggregation is typically neutral, we could imagine finding more and more richness in the internal dynamics. While this does not seem to happen in practice, we do not have an argument why this cannot happen.
	
	What we are saying is that any material is \emph{at least} a minimal material and will possess the dynamic of one coupled with the internal dynamics.
...
Being able to identify particles through either state or kinematic variables is not enough: we also need to be able to express the distributions of the composite states over position and velocity. As such, the differentiable distribution over state variables must become a differentiable distribution over kinematic variables. This tells us the transformation is differentiable.
...
To sum up, the kinematic assumption has more inherent consequences one would expect at first. It means the particles are identified by position and velocity and therefore the system of equations is second order. It links the space of trajectories on $T\mathcal{M}$ with the space of states on $T^*\mathcal{M}$. Such link allows state space metric $\omega$ to induce the space-time metric $g$. It allows us to define Lagrangians. It constrains the motion to massive particles under scalar and vector potential forces. Moreover, it allowed to re-understand basic concepts, like the invariant speed $c$, the inertial mass $m$ and the force tensor $F_{\alpha\beta}$ in terms of state counting possibilities and d.o.f. independence.
