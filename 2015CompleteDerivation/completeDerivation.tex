...
It is time to turn these concepts into a precise mathematical formulation following the template used for classical mechanics. We want to formalize the state space for an \emph{irreducible material} as a quantum particle will be a finite amount of such material.

As for the classical material, the irreducible material is thought to be decomposable: each state can be seen as the sum of different components (e.g. spatially distributed parts) such that the evolution of the whole is the composition of their evolution. Therefore \ref{prop:abelian_group} applies and the state space $\Psi$ of an irreducible material is an abelian group.

Note, though, that the components are in general not independent. As the motion of each fragment is statistical in nature, it will combine as statistical variables do. Suppose $X$ and $Y$ are two random variables,
\begin{equation}\label{eqn:correlation}
\sigma^2_{X+Y} = \sigma^2_{X} + \sigma^2_{Y} + 2 \, \sigma_{X} \sigma_{Y} \rho_{X,Y}
\end{equation}
where $\sigma^2$ is the variance, $\sigma$ is the standard deviation, and $\rho_{X,Y}=COV(X,Y)/\sigma_{X} \sigma_{Y}$ is the Pearson correlation coefficient. As $-1\leq\rho_{X,Y}\leq+1$, the total variance can be greater or smaller than the sum of the variance of the components. The strength of the chaotic motion of the fragments is something physically distinguishable as it contributes to the spread of the distribution. Therefore while the motion of each fragment is not itself physically distinguishable, it falls within the unstated part, the strength and correlation of the chaotic motion must be, they fall within the state.

How can we characterize the aspects of the unstated part that are captured by the state? We can look at the classical derivation for guidance. In that case, we looked at the set of transformations that increase/decrease the amount of material. We can think of them as the transformations that change the unstated part of the system. As the classical material is homogeneous, permutations between classical particles in the same state $\mathcal{s}$ do not matter (i.e. the ordering is unstated). Increasing/decreasing the amount of material increases/decreases the number of permutations and therefore changes the unstated part of the system. For the irreducible material we follow the same strategy: we look for the transformations that affect the unstated part while keeping the overall state, the distribution, unchanged.

Suppose we have $\psi_1$ a state for an amount $a_m$ of irreducible material characterized by one d.o.f. The material will be spread over the configurations of the infinitesimal fragments. As the material is homogeneous, it is spread evenly among such possible configurations. Counting fragment configurations is therefore equivalent to counting the amount of material. Each fragment would be identified by a pair of independent random variables $(A, B)$, as a d.o.f. is two-dimensional. The shape of the distribution and the actual values are not physically distinguishable: these are just dummy variables we assign to count the fragments. We can choose A and B to be uniform distributions of interval $\sqrt{a_m}$, so that they identify a square of area $a_m$, the amount of material.

Suppose we have another configuration $\psi_2$, where only the unstated part is changed. The unstated part of $\psi_2$ can also be characterized by a square formed by two independent uniform distributions (C, D). Counting states on (C,D) is equivalent to counting them on (A,B), as they are simply different arrangements of fragments within the same state. The relationship between the two pairs of variables must be linear since all distributions are uniform. That is: $C=aA+bB$ and $D= -bA + aB$. We can write $\psi_2 = \tau(a,b) \psi_1$ as $a$ and $b$ are the only parameters of the transformation. Or more compactly, $\psi_2 = \tau(c) \psi_1$ where $c=a+\imath b$.

The transformation $\tau(c)$ increases/decreases the area by the modulus $|c|^2=a^2+b^2$, which physically corresponds to the increase/decrease of amount of material. It also rotates the square by the phase $\theta_c \equiv arg(c) = arctan(a/b)$, which corresponds to a change in correlation: if $\theta_c = 0$, $C = |c| A$ are clearly correlated while if $\theta_c = \pi$, $C = - |c| A$ are clearly anti-correlated.

To better understand the link between phase and correlation, suppose $\psi$ is the state for an irreducible material. We can write $\tau(c) \psi = \tau(c_1 + c_2) \psi = \tau(c_1) \psi + \tau(c_2) \psi$ as the composition of two separate transformations. We have
\begin{equation}\label{eqn:correlation_state}
|c|^2=|c_1|^2 + |c_2|^2 + 2 |c_1||c_2|\cos(\theta_{c_2} - \theta_{c_1})
\end{equation}
Compare to \ref{eqn:correlation}. The material is spread evenly among fragment configurations. Also, since it is homogeneous, the ratio of fragments per configuration is the same. Therefore the amount of material is proportional to the strength of the chaotic motion of the fragment, which is measured by the variance. The increase/decrease in amount of material therefore combines like the variance of a statistical variable. The cosine of the phase difference takes the place of the Pearson correlation coefficient. A more meaningful name for the phase, then, would be \emph{correlation angle}. Also note how correlation for a single variable is a meaningless quantity: it is the correlation between two random variables that is significant. That is why in quantum mechanics phase differences have physical meaning, as it gives the correlation between two states, while absolute phases do not.

If we increase the number of independent d.o.f. the group of transformations remains the same.  As the material is homogeneous, the same amount of material needs to be spread over the same number of fragment configurations along each independent d.o.f. This means that, as we combine components, the strength of the chaotic motion has to be the same in all directions. This means that the increase/decrease of material and the change in correlation has to be the same across all d.o.f. 

Mathematically, the state space $\Psi$ for an irreducible material is a complex vector space. It is so because it is the state space for a decomposable material (i.e. an abelian group) where we can increase/decrease the amount and change the correlation of the internal stochastic motion (i.e. a set of transformations conveniently parametrized by a complex number).
...
While $\mathcal{Q}$ is enough to define the system of units, it cannot be enough to define all the variables over which we can define the distributions of irreducible material. The conjugate variables $p_i=\hbar k_i$ cannot be expressed as a function of $q^i$, which reiterates the idea that we cannot specify all variables for each fragment. Suppose there is another set of state variables that provide another maximal description for the fragments. Then those variables will form a topological space $\hat{\mathcal{Q}}$ such that each $\psi$ can be expressed as a wave function $\psi(\hat{q}) :\hat{\mathcal{Q}} \rightarrow \mathbbm{C}$ over $\hat{\mathcal{Q}}$.

As each state $\psi$ can be identified by a wave function on either space, there must be an operator $\Upsilon^q_{\hat{q}}(\psi(q)) = \psi(\hat{q})$ that converts the wave function on one space to the other.\footnote{Note that a neighbourhood of $q^i$ is not necesserily a neighbourood of $\hat{q}^i$. They are only if $q^i$ and $\hat{q}^i$ can be expressed as a bijection of each other $\hat{q}^i(q^i)$: they are different coordinates of the same manifold.} As to a linear combination of states corresponds a linear combination of wave functions, $\Upsilon^q_{\hat{q}}$ is a linear operator. As converting the space again must give us the original wave function, $\Upsilon^{\hat{q}}_{q} \Upsilon^q_{\hat{q}}= 1$ is unitary. We can now define a have linear operator $F_{[\hat{q}^i]}$ corresponding to state variable $f : \hat{\mathcal{Q}} \rightarrow \mathbbm{R}$. Using $\Upsilon^{q}_{\hat{q}}$ we can express $F_{[q^i]} = \Upsilon^{\hat{q}}_{q} F_{[\hat{q}^i]} \Upsilon^{q}_{\hat{q}}$ as an operator over complex functions of $\mathcal{Q}$. It will be still be linear and hermitian.
...
It should be clear, though, how the correspondence between mathematical and physical objects is not as satisfying as in the classical case. For instance, while any differentiable function of $T^*\mathcal{Q}$ can be a state variable for a classical particle, not all self-adjoint linear operators can be associated with a fragment variable. Consider $f: \mathbbm{R} \rightarrow \mathbbm{R}$ for which $f(r) = -r$ if $r$ is an integer and $f(r) = r$ otherwise. Clearly $f(q)$ does not preserve the topology and cannot be used as a fragment variable. Yet, we can still create a self-adjoint linear operator, with the same spectra as the one associated with $q$, just with the eigenvalues switched. It is not clear what plays the role of the differentiable structure of $T^*\mathcal{Q}$ in the quantum case.

The situation is worsened by the fact that the topology of $\mathcal{P}$, the space charted by conjugate momentum, depends globally on the topology of $\mathcal{Q}$. For example, if $\mathcal{Q}$ is an n-dimensional torus, there is no infinity and $\mathcal{P}$ has a discrete topology. That is why, in the end, we restricted ourselves to $\mathbbm{R}^n$. Moreover, we obtained the space of functions not because we required them to fall off at infinity, but because we required the ability to express polynomials of $q^i$ and $p_i$. It is not immediately clear how this generalizes over a manifold.
...
\begin{rationale}
	...
	The problem of incompatibility of measurements and deterministic evolution at all levels still remains. The unstated part of an irreducible system does not change the picture: it is completely undetermined and cannot be used to pass information from one system to another.
	
	The inability to define time using a deterministic and reversible evolution is still a problem. The unstated part cannot give us a measurable quantity that changes in time as that would be a description of the unstated part.
	
	There is one additional problem, though, not present in the classical case. Quantum particle states, as we have defined them, are states at equilibrium: they all provide the same level of description for the fragments. In fact if the internal dynamics were out of equilibrium, the material would not be homogeneous which would contradict the assumption. Deterministic and reversible evolution (which we'll see corresponds to unitary evolution) is necessarily quasi static: at each moment a state must be well defined, at each moment the system is in equilibrium. Therefore quantum states cannot and should not be expected to provide a description under non deterministic evolution.
