...
Naturally, we cannot give a complete description of the configuration of the fragments, therefore we cannot give a joint distribution for all fragment variables at the same time. That is: irreducibility contains the notion of complementarity. We can, though, find a maximal set of fragment variables: one that provides the best description possible. While it should be clear that there cannot be a single unique set, we can show that one such set is provided by the fragment variables $q^i : \mathcal{Q} \rightarrow \mathbbm{R}$ that define the units required to describe the system.

In fact, we cannot add a new d.o.f. (as this would mean changing $\mathcal{Q}$) and we cannot add a new fragment variable in an existing d.o.f. (as this would fully specify said d.o.f.). Therefore the set of fragment variable $q^i$ already provide a maximal description for the fragments. This means that a state $\psi$ must tell us for each point in $\mathcal{Q}$ how much material is there and the correlation of the unstated part. That is: the state will be identified by $\psi : \mathcal{Q} \rightarrow \mathbbm{C}$, a complex function over the $\mathcal{Q}$.
...
\begin{justification}
	...
	We claim $\mathsf{P}_{\psi} (\mathsf{P}_{\phi} (\psi)) = r \psi$ for some $r \in \mathbbm{R}$. Let $\psi, \phi \in \Psi$. Consider $\mathsf{P}_{\psi} (\mathsf{P}_{\phi} (\psi))$: we first take the component of $\psi$ prepared like $\phi$ and then take the component of that prepared like $\psi$. The overall operation is simply returning a part of $\psi$. These must correspond to the same exact configuration for the fragments. The only thing that can change is the amount of material. Therefore $\mathsf{P}_{\psi} (\mathsf{P}_{\phi} (\psi)) = r \psi$ for some $r \in \mathbbm{R}^+$.
...
That is: for each state variable $f$ we have a corresponding linear hermitian operator $F$ which can be expressed as function multiplication in a suitable choice of $\hat{\mathcal{Q}}$ and whose expectations is given by $\langle \psi | F \psi \rangle$.
