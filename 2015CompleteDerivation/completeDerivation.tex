...
\begin{justification}
	We claim there exists a positive linear functional $\Lambda_U : \mathcal{C} \rightarrow \mathbbm{R}$ for each $U \subseteq \mathcal{S}$ for which $\Lambda_U = \Lambda_{\mathrm{int}(U)}$. Let $\Lambda_{U} : \mathcal{C} \rightarrow \mathbbm{R}$ be the functional that returns the amount of material in $\mathcal{c}$ compatible with the outcome associated with the open set $U \subseteq \mathcal{S}$. $\Lambda_U$ is linear. $\Lambda_U(a_1 \mathcal{c}_1 + a_2 \mathcal{c}_2) = a_1 \Lambda_U(\mathcal{c}_1) + a_2 \Lambda_U(\mathcal{c}_2)$ as it has to be consistent with the operations of composing states and increasing/decrease the amount of material by a factor. $\Lambda_U$ is positive. If the value of the distribution for each particle state is positive then the total amount of material is positive. Let $U \subseteq \mathcal{S}$ not necessarily open. Define $\Lambda_U$ as $\Lambda_{\mathrm{int}(U)}$.
	
	We claim that $|\Lambda_{U}(\mathcal{c})| < \infty \; \forall \mathcal{c} \in \mathcal{C} \; \forall U \subseteq \mathcal{S}$. Let $U \subseteq \mathcal{S}$ be an open set of particle states associated with an outcome. Let $\mathcal{c} \in \mathcal{S}$ a composite state. The amount of material of $\mathcal{c}$ associated with the outcome $U$ must be finite, as physically we always work with finite quantities. $|\Lambda_{U}(\mathcal{c})| < \infty$. Let $U \subseteq \mathcal{S}$ not necessarily open. The same argument applies to $\mathrm{int}(U)$.
	...
	We claim that at each $\mathcal{s} \in \mathcal{S}$ there exists a neighborhood $U \subseteq \mathcal{S}$ endowed with a Borel measure $\mu_U$ such that $\Lambda_U (\mathcal{c}) = \int_U \rho_{\mathcal{c}} d \mu_U$. Each $\mathcal{s} \in \mathcal{S}$ is identified by a finite number of independent state variables. $\mathcal{S}$ is a manifold. $\mathcal{S}$ is locally compact. At each point there exists a compact neighborhood $U$. $U$ with the subspace topology is a compact Hausdorff topological space on which is defined a positive linear functional $\Lambda_U$ on the linear vector space $\mathcal{C}$ isomorphic to the space of continuous real valued functions defined on $U$. For the Riesz representation theorem for linear functionals, there exists a unique Borel measure $\mu_U$ such that $\Lambda_U (\mathcal{c}) = \int_U \rho_{\mathcal{c}} d \mu_U$. As $\rho_{\mathcal{c}}$ is continuous $\int_{U} \rho_{\mathcal{c}} d \mu_U = \int_{\mathrm{int}(U)} \rho_{\mathcal{c}} d \mu_U$ which is consistent with $\Lambda_U = \Lambda_{\mathrm{int}(U)}$.
	
	We claim that $\mathcal{S}$ is endowed with a unique Borel measure $\mu$ such that $\Lambda_U (\mathcal{c}) = \int_U \rho_{\mathcal{c}} d \mu$. Let $U \subseteq V \subseteq \mathcal{S}$ two compact subsets. We have $\Lambda_V (\mathcal{c}) = \int_V \rho_{\mathcal{c}} d \mu_V = \int_U \rho_{\mathcal{c}} d \mu_V + \int_{V\backslash U} \rho_{\mathcal{c}} d \mu_V$. We also have $\Lambda_V (\mathcal{c}) = \Lambda_U (\mathcal{c}) + \Lambda_{V\backslash U} (\mathcal{c}) = \int_U \rho_{\mathcal{c}} d \mu_U + \int_{V \backslash U} \rho_{\mathcal{c}} d \mu_{\overline{V \backslash U}} $. As the two have to be equal for all $U, V, \rho_{\mathcal{c}}$, we must have $\int_{U} \rho_{\mathcal{c}} d \mu_U = \int_{U} \rho_{\mathcal{c}} d \mu_V$. Therefore $d\mu$ is unique and does not depend on the choice of neighborhood. Let $\{U_\alpha\}_{\alpha \in A}$ a cover of $\mathcal{S}$ where each $U_\alpha$ is compact. This is possible as $\mathcal{S}$ is locally compact. We have
\begin{align*}
...
\end{align*}
	as $\Lambda_U$ is always finite. The measure is unique on the whole space.
	
	We claim $\mathcal{C} \cong C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$ where $L^1_{ew}(\mathcal{S}, \mu) = \{ \rho : \mathcal{S} \rightarrow \mathbbm{R} \; | \;\; |\int_{U} \rho d\mu| < \infty \; \forall U \subseteq \mathcal{S}\}$. As per \ref{classical_vector space} $\mathcal{C}$, is isomorphic to a subspace $G \subseteq C(\mathcal{S})$ as a vector space over $\mathbbm{R}$. As $|\Lambda_{U}(\mathcal{c})| < \infty \; \forall \mathcal{c} \in \mathcal{C} \; \forall U \subseteq \mathcal{S}$, then $G \subseteq L^1_{ew}(\mathcal{S}, \mu)$. $G \subseteq C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$. Let $\rho_\mathcal{c}$ be the distribution associated with the state $\mathcal{c}$. Let $\rho \in C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$ be a continuous function everywhere integral. If $\rho_\mathcal{c} \neq \rho$ then $\exists U \in \mathcal{S} | \int_{U} \rho_\mathcal{c} d \mu \neq \int_{U} \rho d \mu$. There exist an outcome in $\mathcal{S}$ that can physically distinguish the two distributions. There must be a state $\mathcal{c}_1 \in \mathcal{C} \; | \; \varrho_(\mathcal{c}_1)=\rho$. $\varrho : \mathcal{C} \rightarrow C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$ is surjective. $\varrho$ is an isomorphism between $\mathcal{C}$ and $C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$ as vector spaces.
\end{justification}

As we identified $C(\mathcal{S}) \cap  L^1_{ew}(\mathcal{S}, \mu)$ as the set of distributions that are physically meaningful, it's useful to show how commonly used functions spaces do not fit the bill. We do this briefly by finding counterexample. The set of continuous functions $C(\mathcal{S})$ would include functions that diverge at infinity. The set of continuous functions with compact support $C_C(\mathcal{S})$ would exclude Gaussians. The set of continuous functions that vanish at infinity $C_0(\mathcal{S})$ includes function whose integral is infinite. Schwartz space $S(\mathcal{S})$ requires functions to be infinitely smooth and we do not see a physical requirement for that. 
...
\section{Appendix: mathematical support}
...
\begin{thrm}[Modified Riesz theorem]\label{modified_riesz_theorem}
	Let $\mathcal{L}(S,\mu) \equiv \{ \rho : S \rightarrow \mathbbm{R} \; | \;\; |\int_{U} \rho d\mu| < \infty \; \forall U \subseteq S\}$. $\mathcal{L}(S,\mu)=L^1(S,\mu)$ where $L^1(S,\mu) = \{ \rho : S \rightarrow \mathbbm{R} \; | \;\; \int_{S} |\rho| d\mu < \infty \}$
\end{thrm}
