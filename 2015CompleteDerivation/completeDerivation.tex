...
The first one is that quantum particles are not associated with a well defined value for position and momentum. If we misidentify the electron with a classical particle, this would indeed be worth of note and puzzlement. But if the electron is a classical distribution, there is no issue. The earth is both under Milano and New York. Your right index finger is not, hopefully, at the same position of your left eye. In fact, based on what we saw before, any object of finite mass is a distribution over a region of phase space, and discontinuous distributions are ruled out by \ref{continuous_map}. An electron is an object of finite mass: why should it be any different? In fact, if one claimed an electron to be point-like, he would be claiming an electron behaves in a different way than all other physical objects of finite mass/energy (which are the only ones we experimentally work with). Shouldn't \emph{he} have the burden of proof?
...
The most striking feature, though, is that we can predict the functional form for particle decay over time. Suppose a quantum particle spontaneously decays into two or more particles. Such process cannot be deterministic as the number of d.o.f. increases. As the decay is spontaneous, it is not triggered by the environment (i.e. external state). It must be triggered by the internal dynamic of the fragments. As we assumed the internal dynamic is unknown, we cannot learn anything about it by looking at the product of the decay. In particular, all states have to have equal chance of decay or we could tell that the internal dynamic is more stable for some states. The probability of decay is constant for all states at all times. The probability of survival follows the exponential distribution.
...
\end{equation}
where $\sigma^2$ is the variance, $\sigma$ is the standard deviation, and $\rho_{X,Y}=COV(X,Y)/\sigma_{X} \sigma_{Y}$ is the Pearson correlation coefficient. As $-1\leq\rho_{X,Y}\leq+1$, the total variance can be greater or smaller then the sum of of the variance of the components. The strength of the chaotic motion of the fragments is something physically distinguishable as it contributes to the spread of the distribution. Therefore while the motion of each fragment is not itself physically distinguishable, it falls within the unstated part, the strength and correlation of the chaotic motion must be, they fall within the state.

How can we characterize the aspects of the unstated part that are captured by the state? We can look at the classical derivation for guidance. In that case, we looked at the set of transformation that increase/decrease the amount of material. We can think them as the transformations that change the unstated part of the system. As the classical material is homogeneous, permutations between classical particles in the same state $\mathcal{s}$ do not matter (i.e. the ordering is unstated). Increasing/decreasing the amount of material increases/decreases the number of permutations and therefore changes the unstated part of the system. For the irreducible material we follow the same strategy: we look for the transformations that affect the unstated part while keeping the overall state, the distribution, unchanged.

Suppose we have $\psi_1$ a state for an amount $a_m$ of irreducible material characterized by one d.o.f. The material will be spread over the configurations of the infinitesimal fragments. As the material is homogeneous, the material is spread evenly among such possible configurations. Counting fragment configurations is therefore equivalent to counting amount of material. Each fragment would be identified by a pair of independent random variables $(A, B)$, as a d.o.f. is two-dimensional. The shape of the distribution and the actual values are not physically distinguishable: these are just dummy variables we assign to count the fragments. We can choose A and B to be uniform distributions of interval $\sqrt{a_m}$, so that they identify a square of area $a_m$, the amount of material.
...
The transformation $\tau(c)$ increases/decreases the area by the modulus $|c|^2=a^2+b^2$, which physically corresponds to the increase/decrease of amount of material. It also rotates the square by the phase $\theta_c \equiv arg(c) = arctan(a/b)$, which corresponds in a change in correlation: if $\theta_c = 0$, $C = |c| A$ are clearly correlated while if $\theta_c = \pi$, $C = - |c| A$ are clearly anti-correlated.
...
\end{equation}
Compare to \ref{eqn:correlation}. The material is spread evenly among fragment configurations. Also, since it is homogeneous, the ratio of fragment per configuration is the same. Therefore the amount of material is proportional to the strength of the chaotic motion of the fragment, which is measured by the variance. The increase/decrease in amount of material therefore combines like the variance of a statistical variable. The cosine of the phase difference takes the place of the Pearson correlation coefficient. A more meaningful name for the phase, then, would be \emph{correlation angle}. Also note how correlation for a single variable is a meaningless quantity: it is the correlation between two random variables that is significant. That is why in quantum mechanics phase differences have physical meaning, as it gives the correlation between two states, while absolute phases do not.
...
Mathematically, the state space $\Psi$ for an irreducible material is a complex vector space. It is so because it is the state space for a decomposable material (i.e. an abelian group) where we can increase/decrease the amount and change the correlation of the internal stochastic motion (i.e. a set of transformation conveniently parametrized by a complex number).
