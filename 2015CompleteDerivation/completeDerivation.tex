...
\begin{assump}[Infinitesimal reducibility]\label{classical}
...
\begin{justification}
	We claim $\mathcal{T}_{\Delta t}$ is a self-homeomorphism on $T^*\mathcal{Q}$. The state space for the particles of a classical material is $T^*\mathcal{Q}$ by \ref{classical_phase_space}. $\mathcal{T}_{\Delta t}$ is a deterministic and reversible evolution map and by \ref{detrevmap} is a self-homeomorphism.
...
The first thing to do is to amend our definition of state space to include all states at all times. It may seem like we are extending our definition of state and state space but, at a closer look, we are not. Since defined state as a physical configuration at a particular time, the set of all states (i.e. the state space) is more accurately the set of all configurations at all times. In the previous sections we tacitly simplified the problem by ignoring time, which made it easier to study the time independent case. This is common in physics and engineering but this is also the source of the above problems.

We'll therefore call \emph{complete state space} the set of all configurations at all times and we'll indicate it by $\bar{\mathcal{S}}$. In that space, a state is an element $\mathcal{s} \in \bar{\mathcal{S}}$ while a time evolution is a line $\bar{\mathcal{s}} \subset \bar{\mathcal{S}}$ that passes once at all times. Specifying an evolution map means giving a set of lines such that each state is traversed once and only once. Note that the transported state variables are perfect to label the evolutions: as they do not change during the evolution we can take them to label not just the values at a particular time, but the evolutions themselves.

With this geometric picture in mind, we can understand better the complete state space $\bar{\mathcal{C}}$ for the composite system. An evolution $\hat{\mathcal{c}}$ will be a distribution of material over all configuration at all times. Along a particle state evolution $\bar{\mathcal{s}}$, though, the distribution is constant as all the material is transported deterministically and reversibly. A state space $\mathcal{c}$, then, is a distribution over an hyper-surface that cuts across all the particle state evolutions. If we think it as a distribution over the transported state variables we are guaranteed to cover all initial conditions no matter the hyper-surface.
