...
We claim $\mathcal{C}$ is isomorphic to a subspace of $C(\mathcal{S})$ as a vector space over $\mathbbm{R}$. Let $\rho:\mathcal{S} \rightarrow \mathbbm{R}$ be a map returning the amount of material for each state $\mathcal{s} \in \mathcal{S}$. $\rho \in C(\mathcal{S})$ by \ref{continuous_map}. Let $G \subset C(\mathcal{S})$ be the functions that correspond to physically meaningful configurations. To each $\mathcal{c} \in \mathcal{C}$ corresponds one and only one $\rho_\mathcal{c} \in G$ as the system is infinitesimally reducible. Let $\mathcal{c}=\mathcal{c}_1+\mathcal{c}_2$, then $\rho_\mathcal{c}=\rho_{\mathcal{c}_1}+\rho_{\mathcal{c}_2}$ as the amount of material of the composition of two states is the sum of the individual amounts. Let $\mathcal{c}_1=\tau(a)\mathcal{c}_2$, then $\rho_{\mathcal{c}_1}=a\rho_{\mathcal{c}_2}$ as $\tau(a)$ increases the amount of material by the factor $a$. $\mathcal{C}\cong G$ as vector spaces. $G$ is a subspace of $C(\mathcal{S})$.

\end{justification}

Note that not all continuous functions correspond to physically meaningful states: we must make sure they describe a finite amount of material.

\subsection{Coordinate invariant densities}

For each classical composite state will also tell us the total amount of material, once we fixed unit (e.g. mole, kg, ...). That is, there will be a functional $\Lambda : \mathcal{C} \rightarrow \mathbbm{R}$ that tells us how much gas we have for a particular gas configuration. As we combine parts, the total amount of material will be the sum of their parts. As we increase or decrease by a constant factor the amount of material will increase or decrease by the same factor. That is: $\Lambda(a_1 \mathcal{c}_1 + a_2 \mathcal{c}_2) = a_1 \Lambda(\mathcal{c}_1) + a_2 \Lambda(\mathcal{c}_2)$. For a proper state, one that is not the difference of two states, we will expect a positive amount of material. So, if a density $\rho(\mathcal{S})$ is positive everywhere, then the total amount is also positive. This makes $\Lambda$ a positive linear functional.

Being able to tell the total amount is not enough. Experimentally, what we do is measure the amount of material for a region of phase space. For example, the amount of gas within a particular region of space for a particular range of momentum. In general, given an outcome of our physical process (i.e. an open set in the topology for $\mathcal{S}$), each composite state should tell us the amount of material associated with it (i.e. contained in the open set of states). For each $U \subseteq \mathcal{S}$ we'll have a positive linear functional $\Lambda_U : \mathcal{C} \rightarrow \mathbbm{R}$ whose value is always bounded.

The ability to associate finite amount of material with sets of states allows us to define finite regions of $\mathcal{S}$ and compare them. Consider a region of position and momentum in phase space. If we are able to spread a finite amount of material into a uniform distribution, then we know we have a finite region. And the density will give us an indication of how big the region is: if we double the region, the density will half. In other words: because we are describing densities over $\mathcal{S}$, we are able to give a unique way to measure the size of sets of $\mathcal{S}$, a metric $d\mu$ and our positive linear functionals become integrals: $\Lambda_U (\mathcal{c}) : \int_U \rho_{\mathcal{c}} d \mu$.

In mathematical terms, we can use the Riesz representation theorem for linear functionals to give $\mathcal{S}$ the structure of a measure space, with a suitable Borel $\sigma$-algebra and measure $\mu$.
